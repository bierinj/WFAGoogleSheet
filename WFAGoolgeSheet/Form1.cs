namespace WFAGoolgeSheet
{
    ...
    public partial class Form1 : Form
    {
        ...
        /// </summary>
        public static bool GPSfenceDone = false;
        ...
        public void BuildComboBoxItems(Int32 defIndex)
        {
            ...
            if (radioButton6.Checked)
            {
                ...
                comboBox1.Items.Add(new sTabName { langName = "Names of Deaf Lost", tabname = "Names of Deaf Lost", range = "!A1:E", keyfield = "TELEPHONE" });
            }
            else
            {
                ...
                comboBox1.Items.Add(new sTabName { langName = "Nombres de sordos perdidos", tabname = "Names of Deaf Lost", range = "!A1:E", keyfield = "TELEPHONE" });
        ...
        /// <summary>
        /// The GetFirstDateFromString.
        /// </summary>
        /// <param name="inputText">The inputText<see cref="string"/>.</param>
        /// <returns>The <see cref="DateTime?"/>.</returns>
        internal static DateTime? GetFirstDateFromString(string inputText)
        {
        ...
        protected bool CheckDate(String date)
        {
            ...
            String[] inputText = date.Split(' ');//split on a whitespace
        ...
        private static void seeVisibleRow(DataGridView view, int rowToShow)
        {
            if (rowToShow >= 0 && rowToShow < view.RowCount)
            {
                ...
                {
                    for (; x < countVisible; x++)
                    {
                        if (view.Rows[rowToShow - (countVisible - x)].Visible == true)
                            break;
                    }
                    view.FirstDisplayedScrollingRowIndex = rowToShow - (countVisible - x);
                }
                view.Enabled = false;
            }
            view.Refresh();
       ...
       CultureInfo cultureInfo)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            dataGridView1.DefaultCellStyle.SelectionForeColor = System.Drawing.SystemColors.WindowText;
            dataGridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dataGridView1.CurrentCellChanged += new EventHandler(HandleCurrentCellChanged);
            ...
            if (values != null && values.Count > 0)
            {
                ...
                request2.Ranges = string.Format("{0}!A{1}:B{2}", selectCar.tabname, rowOffset + 1, values.Count);
                ...
                for (int b = 0; b < values.Count - rowOffset; b++)
                {
                    for (int k = 0; k < 2; k++)
                    {
                        ...
                        var gval = (float)0.0;
#if SheetColors == true
                        if (response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Blue == null) bval = (float)0.0;
                        else bval = (float)response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Blue;
                        if (response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Red == null) rval = (float)0.0;
                        else rval = (float)response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Red;
                        if (response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Green == null) gval = (float)0.0;
                        else gval = (float)response2.Sheets[0].Data[0].RowData[b].Values[k].EffectiveFormat.BackgroundColor.Green;
                ...
                dataGridView1.ClearSelection();
                NumofRec = 0;
                int c = 0;          // index for color look up
                //
                ...
                foreach (var row in values)
                {
                    ...
                    for (int z = 0; z < row.Count; z++)
                    {
                        ...
                        if (z == 0 || z == 1)
                        {
                            ccolor = Program.HexStringToColor(bcolor[c++]);
                            ...
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#fffbbc04"))
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Contacted 5 times letters\"";

                            // Pink   = Exists in "Only Spanish"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ffea9999"))
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Out Terr\"";

                            // Red (~1)   = Exists in "This Sheet"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ffea4335"))
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"This Sheet\"";

                            // Purple = Exists in "Confirmed Deaf"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ff8e7cc3"))
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Confirmed Deaf\"";

                            // Dark Orange (Gold) (~15) = Exists in "Field Service"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ffff6d01"))
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Field Service\"";

                            // Dark Grey (30)  = Exists in "Imported Names"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ff7f6000"))
                            {
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Imported Names\"";
                                ccolor = System.Drawing.Color.SandyBrown; ;             // this RGB color was too dark
                            }

                            // Green (30)  = Exists in "Names of Deaf Lost"
                            if (ccolor == System.Drawing.ColorTranslator.FromHtml("#ff41c941"))
                            {
                                dataGridView1.Rows[x].Cells[z].ToolTipText = "Exists in \"Names of Deaf Lost\"";
                            }
                            ...
                            dataGridView1.Rows[x].Cells[z].Style.BackColor = ccolor;
                        }
#endif
                    }
            ...
            for (int y = 0; y < dataGridView1.Rows.Count; y++)
                ...
                {
                    dataGridView1.Rows[y].Cells[GetindexOf(dataGridView1, "ADDRESS")].Selected = true;
                    break;
                }
                    ...
                    if (selectIndex == 1)               // Field Service
                    {
                        ...
                        if (selectIndex == 1)           // is Field Service selected
                        {
                            using (var Form2 = new Form2())
                            {
                                ...
                                form2.textBox2.BackColor = dataGridView1.Rows[nRow].Cells[1].Style.BackColor;
                                myToolTip.SetToolTip(form2.textBox2, dataGridView1.CurrentRow.Cells[1].ToolTipText);
                                ...
                                //
                                // setup notes box with current date & time 
                                // and comments (translated if needed)
                                //
                                ...
                                string ttemp = string.Join(System.Environment.NewLine, form2.textBox6.Text.Split('^'));

                                //
                                // see if notes have todays date already. Add if not
                                //
                                try
                                {
                                    for (int j = 0; j < lines.Length; j++)
                                    {
                                        if (string.IsNullOrEmpty(lines[j]) && j < lines.Length) continue;
                                        if (CheckDate(lines[j])) dateValue = (DateTime)GetFirstDateFromString(lines[j]);
                                        else continue;
                                        if (DateTime.Parse(dateValue.ToString("yyyy-MM-dd")) == DateTime.Parse(today.ToString("yyyy-MM-dd")))
                                        {
                                            firstpos = true;
                                            break;
                                        }
                                    ...
                                    if (!firstpos)
                                    {
                                        var dt = DateTime.Now;
                                        if (radioButton5.Checked)
                                            ndt = dt.ToString(format, new CultureInfo("es-ES"));
                                        if (radioButton6.Checked)
                                            ndt = dt.ToString(format, new CultureInfo("en-US"));
                                    }
                                }
                                catch
                                {
                                    if (radioButton6.Checked)
                                        form2.textBox6.Text = form2.textBox6.Text + Environment.NewLine + "(en):" + ndt + ": ";
                                    if (radioButton5.Checked)
                                        form2.textBox6.Text = form2.textBox6.Text + Environment.NewLine + "(es):" + ndt + ": ";
                                }

                                //
                                // prepare to translate notes
                                //
                                newlines.Clear();
                                var ltext = "";
                                string totext = "";
                                var task = (dynamic)null;
                                string ttext = null;
                                ...
                                lb2: if (lines[j].Contains("(es)") || lines[j].Contains("(en)"))
                                    {
                                        // if running in spanish and note is in english
                                        if (radioButton5.Checked && lines[j].Contains("(en)"))
                                        {
                                            if ((lines.Length > j + 1) && (!lines[j + 1].Contains("(es)")))
                                            {
                                                // save existing line
                                                newlines.Add(lines[j]);
                                                // insert new translated line
                                            ...
                                            else
                                            {
                                                // save existing line
                                                newlines.Add(lines[j]);
                                                // test if next line has the translation and translate if not
                                                if ((lines.Length > j + 1) && (lines[j + 1].Contains("(es)") || lines[j + 1].Contains("(en)")))
                                                {
                                                    newlines.Add(Environment.NewLine + lines[++j]);
                                                    continue;
                                                }
                                                else
                                                {
                                        ...
                                        else if (radioButton6.Checked && lines[j].Contains("(es)"))
                                        {
                                            if ((lines.Length > j + 1) && (!lines[j + 1].Contains("(en)")))
                                            {
                                                // save existing line
                                                newlines.Add(lines[j]);
                                                // insert new line
                                                task = Program.gTranslate(lines[j], "es", false);
                                                task.Wait();
                                                ttext = task.Result.ToString();
                                    ...
                                    if (lines.Length > j + 1 && lines[j + 1].Contains("(es)") && CheckDate(lines[j + 1]))
                                    {
                                        j++;
                                        continue;
                                    }
                                    if (lines.Length > j + 1 && lines[j + 1].Contains("(en)") && CheckDate(lines[j + 1]))
                                    {
                                        j++;
                                        continue;
                                    }
                                           ...
                                           ((ltext == "en") && radioButton5.Checked))
                                    {
                                        ...
                                        form2.textBox6.AppendText(Environment.NewLine + "(" + totext + "):" + ttext);
                                        form2.textBox6.Update();
                                    }
                                    else
                                    {
                                        newlines.Add(lines[j]);
                                ...
                                {
                                    if (radioButton6.Checked)
                                        form2.textBox6.Text = form2.textBox6.Text + "(en):" + ndt + ": ";
                                    if (radioButton5.Checked)
                                        form2.textBox6.Text = form2.textBox6.Text + "(es):" + ndt + ": ";
                                }

                                switch (dataGridView1.CurrentRow.Cells[GetindexOf(dataGridView1, "RESULTS")].Value?.ToString())
                                {
                                    case "N/A":
                                    ...
                                    try
                                    {
                                        ...
                                        while (nRow < dataGridView1.RowCount - 1)
                                            if (nRow < dataGridView1.RowCount)
                                            {
                                                ...
                                                int currentRow = nRow;
                                                dataGridView1.Rows[currentRow].Cells[GetindexOf(dataGridView1, "TELEPHONE")].Selected = true;
                                                dataGridView1.Refresh();
                                                break;
                                            }
                                    }
        ...
        /// <summary>
        /// The DataGridView1_CellMouseClick.
        /// </summary>
        /// <param name="sender">The sender<see cref="Object"/>.</param>
    ...
    object sender, DataGridViewCellMouseEventArgs e)
        ...
        /// <summary>
        /// The HandleRowPostPaint.
        /// </summary>
        /// <param name="sender">The sender<see cref="object"/>.</param>
        /// <param name="e">The e<see cref="DataGridViewRowPostPaintEventArgs"/>.</param>
        private void dataGridView1_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            dataGridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            if (dataGridView1.Rows[e.RowIndex].Selected)
            {
                using (Pen pen = new Pen(System.Drawing.Color.SlateGray))
                {
                    int penWidth = 3;
    ...
    DataGridViewRowCancelEventArgs e)
        ...
        /// The FixDateTime.
        /// </summary>
        /// <param name="ttext">The ttext<see cref="string"/>.</param>
        /// <returns>The <see cref="string"/>.</returns>
        ...
        private void getAllPhoneNumbers(SheetsService sheetService, string spreadsheet)
        {
            ...
            //
            string range = "Confirmed Deaf!A5:A";
            SpreadsheetsResource.ValuesResource.GetRequest request =
            ...
            foreach (var row in values)
            {
                ...
                names2chk[i].Add("Confirmed Deaf".ToString());
            ...
            if (lastCErow > 0)
            {
                foreach (var row in values)
                {
                    ...
                    names2chk.Add(new List<String>());
                    names2chk[i].Add(forchk); //Add values to the sub List at index 0
                    names2chk[i].Add("Out Terr".ToString());
                    if (names2chk.Distinct().Count() != names2chk.Count())
                    {
                        if (radioButton6.Checked)
                            System.Windows.Forms.MessageBox.Show("duplicate {0}", forchk.ToString());
        ...
        /// <summary>
        /// The GetSelectedReport.
        /// </summary>
        /// <param name="parent">The parent<see cref="Control"/>.</param>
        /// <returns>The <see cref="RadioButton"/>.</returns>
        private RadioButton GetSelectedReport(Control parent)
        {
            return parent.Controls.OfType<RadioButton>().Where(c => c.Checked).FirstOrDefault();
        }

        /// <summary>
        /// The HandleCurrentCellChanged.
        /// </summary>
        /// <param name="sender">The sender<see cref="object"/>.</param>
        /// <param name="e">The e<see cref="EventArgs"/>.</param>
        private void HandleCurrentCellChanged(object sender, EventArgs e)
        {
            if (oldCellY != -1)
            {
                dataGridView1.InvalidateRow(oldCellY);
            }
            oldCellY = dataGridView1.CurrentCellAddress.Y;
        }

        /// <summary>
        /// The HandleRowPrePaint.
        /// </summary>
        /// <param name="sender">The sender<see cref="object"/>.</param>
        /// <param name="e">The e<see cref="DataGridViewRowPrePaintEventArgs"/>.</param>
        ...
        {
            Form2 form2 = new Form2();
            //Form2 form2 = (Form2)System.Windows.Forms.Application.OpenForms["Form2"];
            string cellResult = "";
            ...
            //
            //var rb =  GetSelectedReport(form2.groupBox1);
            //int rbs = int.Parse(rb.ToString());
            switch (checkedRadio)
            {
                case "1":     // "No Ans.":
                    cellResult = "N/A";
                ...
                case "5":     // "Deaf in family":
                    cellResult = "dF";
                    break;

                case "6":     // "Deaf Neighbor":
                    cellResult = "dN";
                    break;
        ...
        public class MyClass2
        {
            ...
            /// Defines the GPSfenceDone.
            /// </summary>
            public static int GPSfenceDone;
        }
